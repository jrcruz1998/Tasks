...
class TaskViewHolder(private val itemBinding: RowTaskListBinding, val listener: TaskListener) :
    ...
    fun bindData(task: TaskModel) {
        ...
        // Converter o formato do dueDate
        val date = SimpleDateFormat("yyy-MM-dd").parse(task.dueDate)
        itemBinding.textDueDate.text = SimpleDateFormat("dd/MM/yyyy").format(date)

        // Mudar a imagem caso a tarefa esteja completa
        if (task.complete) {
        ...
        // Eventos
        itemBinding.buttonEdit.setOnClickListener { listener.onListClick(task.id) }
